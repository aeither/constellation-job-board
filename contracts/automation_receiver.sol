...
contract BumpUpkeep is ILogAutomation {
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        performData = abi.encode(logSender, number, log.txHash, log.timestamp);
        return (true, performData);
    }

    function performUpkeep(bytes calldata performData) external override {
        (address logSender, uint256 number, bytes32 txnHash, uint256 timestamp) = abi.decode(
            performData,
            (address, uint256, bytes32, uint256)
        );
        counter += 1;
        emit Bumped(logSender, msg.sender, number, block.number, counter, txnHash, timestamp);
    }
